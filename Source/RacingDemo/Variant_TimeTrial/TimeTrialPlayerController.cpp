...
void ATimeTrialPlayerController::BeginPlay()
{
	...
	if (IsLocalPlayerController())
	{
		...
		if (UIWidget)
		{
			...
			UE_LOG(LogRacingDemo, Error, TEXT("Could not spawn Time Trial UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	SportsCarPawn = CastChecked<ARacingDemoSportsCar>(InPawn);
...
void ATimeTrialPlayerController::Tick(float Delta)
{
	...
	if (IsValid(VehiclePawn) && IsValid(VehicleUI))
	{
		...
		//---my code---
		if (SportsCarPawn)
			UIWidget->SetHealth(SportsCarPawn->GetHealth());
		//---end of my code---
	}
}
